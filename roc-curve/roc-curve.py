...
def roc_curve(y_true, y_score):
    ...
    # sort by descending score: break ties by putting positive first
    # np.lexsort sorts by the LAST key first, so
    #  primary key = y_score (descending -> negate)
    #  secondary key = y_true (descending -> negate )
    ...
    # cumulative true positives at every possible threshold
    ...
    # keep only positions where the threshold actually changes
    # a new threshold occurs where the score changes to a lower value
